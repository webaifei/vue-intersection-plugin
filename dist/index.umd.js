...
(function (global, factory) {
  ...
  (global = global || self, global.VueIntersection = factory());
  ...
  VueIntersection.install = function (Vue, options) {
    var $intersection = new VueIntersection(Vue, options);
    var that = $intersection;
    Vue.prototype.$intersection = $intersection; // 添加指令
  ...
  VueIntersection.prototype._observe = function () {
    ...
    var _this$options = this.options,
        ...
        rootMargin = _this$options.rootMargin,
        ...
        if (item.intersectionRatio > threshold[0]) {
          ...
          if ($el.preIntersectionRatio < threshold[0]) {
            ...
            if (Math.abs($el.preIntersectionRatio - threshold[0]) <= threshold[0] / 2) {
              ...
              // leave
              // try clearTimeout
              var timeInterval = item.time - $el.preTime; // console.log(timeInterval, duration, $el.$$timer, "clear timeout")
  ...
  VueIntersection.prototype._startTimer = function ($el, handler, duration) {
    $el.$$timer = setTimeout(function () {
      if (typeof handler === "function") {
        var logConfigStr = $el.dataset.log;
